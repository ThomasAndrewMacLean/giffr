...
class Resolver {
  ...
  resolve(pathnameOrContext) {
    ...
    function next(resume) {
      let parent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : matches.value.route;
      let prevResult = arguments.length > 2 ? arguments[2] : undefined;
...
function createLocation(_ref, route) {
  ...
  return {
    ...
    getUrl: function getUrl() {
      let userParams = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
...
class Router extends Resolver {
  ...
  setRoutes(routes) {
    let skipRender = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  ...
  render(pathnameOrContext, shouldUpdateHistory) {
    ...
    const {
      ...
      search: '',
      hash: ''
    } : pathnameOrContext; // Find the first route that resolves to a non-empty result
  ...
  __fullyResolveChain(topOfTheChainContextBeforeRedirects) {
    let contextBeforeRedirects = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : topOfTheChainContextBeforeRedirects;
  ...
  __findComponentContextAfterAllRedirects(context) {
    ...
    if (result instanceof HTMLElement) {
      ...
      return this.__redirect(result.redirect, context.__redirectCount).then(context => this.__findComponentContextAfterAllRedirects(context));
  ...
  __isReusableElement(element, otherElement) {
    if (element && otherElement) {
      return this.__renderedElements.get(otherElement) ? element.localName === otherElement.localName : element === otherElement;
    }
  ...
  __ensureOutlet() {
    let outlet = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.__outlet;
  ...
  __updateBrowserHistory(_ref2, replace) {
    let {
      pathname,
      search = '',
      hash = ''
  ...
  __addAppearingContent(context, previousContext) {
    ...
    this.__disappearingContent = Array.from(deepestCommonParent.children).filter(e => e !== context.result); // Add new elements (starting after the deepest common parent) to the DOM.
...
const runIfDevelopmentMode = function runIfDevelopmentMode(callback, args) {
...
const usageStatistics = function usageStatistics() {
